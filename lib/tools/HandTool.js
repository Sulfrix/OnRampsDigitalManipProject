class HandTool extends Tool {
    ...
    draw() {
        ...
        if (this.dragging) {
            var snapResult = this.dragging.snapToPart(null, true)
            if (snapResult.to && snapResult.from) {
                var arrowSize = 10
                var to = snapResult.to.getPos();
                var from = snapResult.from.getPos();
                arrowSize = to.dist(from)/5
                var dir = to.copy().sub(from)
                dir.normalize()
                var right = dir.copy().rotate(-PI/2).mult(arrowSize)
                var left = dir.copy().rotate(PI/2).mult(arrowSize)
                var vvert = (vector) => {vertex(vector.x, vector.y)} // TODO: don't do this shit EVERY FRAME
                push();
                noStroke();
                fill(255, 188)
                beginShape();
                vvert(from.copy().add(left))
                vvert(from.copy().add(dir.copy().mult(arrowSize)))
                vvert(from.copy().add(right))
                vvert(to)
                endShape();
                pop();
            }
