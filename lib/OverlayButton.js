class OverlayButton {
    ...
    draw() {
        this.slide = lerp(this.slide, this.targetSlide, 1 - Math.exp(-10*deltaTime/1000))
        if (this.slide <= 0) {
            return 0;
        }
        push();
        ...
        fill(0, 127);
        if (this.mouseHovered()) { // highlight effects
            fill(120, 127)
            if (mouseIsPressed) {
                fill(140, 128);
            }
        ...
        if (!this.key) { // The text draws in a different position if there is no keybind, unusued
