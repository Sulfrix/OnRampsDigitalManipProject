...
pInput.registerCanvas = function (canvas) {
	// So we know what to hook events to
	/**
	 * @type {HTMLCanvasElement}
	 */
	...
	element.addEventListener("mousemove", (e) => {
		if (pInput.workspace) {
			pInput.workspace.mouseMove(e);
		}
	});
	element.addEventListener("contextmenu", (e) => {
		// The right click menu is disabled
		e.preventDefault();
	});
...
pInput.registerWorkspace = function (workspace) {
	// So we know what Workspace to send events to
	/**
	 *
		...
		if (event.composedPath().includes(pInput.canvas)) {
			if (workspace.validMousePoint(mouseX, mouseY)) {
				if (event.ctrlKey || !detectTrackPad(event)) {
					...
					workspace.cameraCenter.add(
						createVector(event.deltaX, event.deltaY).div(
							workspace.zoom
						)
					);
				}
