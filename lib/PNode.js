/// <reference path="../types/p5/global.d.ts"/>

/**
 * @typedef {Object} SerializedPNode
 * @property {string} uuid
 * @property {string} name
 * @property {SerializedBoundingBox} boundingBox
 * @property {SerializedSocket[]} inputSockets
 * @property {SerializedSocket[]} outputSockets
 */

class PNode {
	constructor() {
		this.uuid = crypto.randomUUID()
		this.name = "Node";
		this.boundingBox = new BoundingBox(createVector(), 150, 150);
		this.padding = 9;
		this.innerDetail = 5;
		this.gbuffer = createGraphics(150, 150);
		this.maskbuffer = createGraphics(150, 150);
		this.updateSize();
		this.forceClip = false;

		/**
		 * @type {Workspace}
		 */
		this.workspace = null;

		/**
		 * @type {Socket[]}
		 */
		this.inputSockets = [];
		/**
		 * @type {Socket[]}
		 */
		this.outputSockets = [];

		/**
		 * @type {Socket[]}
		 */
		this.allSockets = [];

		this.shouldRepaint = false;

		// TODO: Prototype code, please remove
		this.addSocket(new Socket("A", "a", this, "number", "i"));
		this.addSocket(new Socket("B", "b", this, "number", "i"));
		this.addSocket(new Socket("Out", "out", this, "number", "o"));
	}

	// TODO: Work with a definition system for nodes, which are referenced in serialization

	/**
	 * 
	 * @returns {SerializedPNode}
	 */
	serialize() {
		// TODO: Don't serialize the whole node...
		return {
			uuid: this.uuid,
			name: this.name,
			boundingBox: this.boundingBox.serialize(),
			inputSockets: this.inputSockets.map((socket) => {return socket.serialize()}),
			outputSockets: this.outputSockets.map((socket) => {return socket.serialize()})
		}
	}

	/**
	 * 
	 * @param {SerializedPNode} data 
	 * @returns {PNode}
	 */
	static createFromData(data) {
		let out = new PNode()
		out.uuid = data.uuid
		out.name = data.name
		out.boundingBox = BoundingBox.createFromData(data.boundingBox),
		out.inputSockets = data.inputSockets.map((dataSocket) => {return Socket.createFromData(dataSocket, out)})
		out.outputSockets = data.outputSockets.map((dataSocket) => {return Socket.createFromData(dataSocket, out)})
		out.shouldRepaint = true
		out.updateSize()
		out.updateSockets()
		return out
	}

	findSocketByID(id) {
		// TODO: use a map for this
		for (let socket of this.allSockets) {
			if (socket.id == id) {
				return socket
			}
		}
	}

	/**
	 *
	 * @param {Socket} socket
	 */
	addSocket(socket) {
		switch (socket.io) {
			case "i":
				this.inputSockets.push(socket);
				break;
			case "o":
				this.outputSockets.push(socket);
				break;
		}
		this.shouldRepaint = true;
		this.updateSockets()
	}

	updateSockets() {
		this.allSockets = concat(this.inputSockets, this.outputSockets)
		for (let socket of this.allSockets) { // Done to set socket positions
			PNode.socketDraw(this, socket, createVector(socket.io == "i" ? -1 : 1, 0).rotate(this.boundingBox.rot))
		}
	}

	get pos() {
		return this.boundingBox.pos;
	}

	get width() {
		return this.boundingBox.width;
	}

	get height() {
		return this.boundingBox.height;
	}

	updateSize() {
		this.gbuffer.resizeCanvas(
			this.width * this.innerDetail,
			this.height * this.innerDetail
		);
		//this.gbuffer.elt.willReadFrequenty = true
		this.maskbuffer.resizeCanvas(
			this.width * this.innerDetail,
			this.height * this.innerDetail
		);
		if (!this.forceClip) {
			this.drawMask(this.maskbuffer);
		}
		this.repaint();
	}

	drawMask(g) {
		//g.clear()
		g.reset();
		g.scale(this.innerDetail);
		g.noStroke();
		g.fill(255);
		let maskpadding = 2.5;
		//g.rect(1, 1, this.width-2, this.height-2, (this.padding*2)-1)
		g.rect(
			maskpadding,
			maskpadding,
			this.width - maskpadding * 2,
			this.height - maskpadding * 2,
			this.padding * 1.5 - maskpadding
		);
	}

	update() {}

	/**
	 * 0 - use Image.mask() (slow)
	 * 1 - use clip() in drawinner() (faster)
	 */
	static maskMethod = 1;

	repaint() {
		this.drawInner(this.gbuffer);
		if (!this.forceClip && PNode.maskMethod == 1) {
			this.masked = this.gbuffer.get();
			this.masked.mask(this.maskbuffer);
		} else {
			this.masked = this.gbuffer;
		}
	}

	draw() {
		if (this.shouldRepaint) {
            PerfTrack.push("Node Repaint")
			this.repaint();
			this.shouldRepaint = false;
            PerfTrack.pop()
		}
		push();

		this.boundingBox.boxTranslate();
		translate(-this.width / 2, -this.height / 2);
		//stroke(0)
		//strokeWeight(2)
		//noFill()
		fill(0);
		if (this.workspace.selectedNodes.indexOf(this) != -1) {
			fill(255);
		}
		noStroke(0);
		rect(0, 0, this.width, this.height, this.padding * 1.5);

		noSmooth();
		let bufimg = this.masked;
		if (this.workspace.zoom > 1.5 || this.forceClip) {
			clip(() => {
				let maskpadding = 2.5;
				rect(
					maskpadding,
					maskpadding,
					this.width - maskpadding * 2,
					this.height - maskpadding * 2,
					this.padding * 1.5 - maskpadding
				);
			});
			bufimg = this.gbuffer;
		}
		scale(1 / this.innerDetail);
		image(bufimg, 0, 0);

		pop();
		this.drawSocketPoints();
	}

	static socketDraw(node, socket, fwd) {
		let drawPos = node.boundingBox.localPosCorner(socket.pos);
		socket.globalPos = drawPos
		socket.forward = fwd
		//ellipse(drawPos.x, drawPos.y, 10, 10, 8)
		rect(drawPos.x - 5, drawPos.y - 5, 10, 10);
	}

	drawSocketPoints() {
		if (this.workspace.zoom < 0.25) {
			return;
		}
		push();
		fill(255);
		stroke(0);
		strokeWeight(1);
		let nodeForward = createVector(1, 0)
		nodeForward.rotate(this.boundingBox.rot)
		for (let socket of this.allSockets) { // i don't like copying the vector here but whatever
			PNode.socketDraw(this, socket, socket.io == "i" ? nodeForward.copy().mult(-1) : nodeForward.copy())
		}
		pop();
	}

	/**
	 *
	 * @param {import("types/p5").Graphics} g
	 */
	drawInner(g) {
		g.clear();
		g.reset();
		g.scale(this.innerDetail);
		if (!this.forceClip && PNode.maskMethod == 0) {
			g.clip(() => {
				this.drawMask(g);
			});
		}
		g.background(80);
		this.drawTitle(g);
		this.drawSocketText(g);
	}

	/**
	 *
	 * @param {import("types/p5").Graphics} g
	 */
	drawTitle(g) {
		g.push();
		g.textSize(14);
		g.noStroke();
		g.textAlign(LEFT, TOP);
		g.fill(80, 0, 0);
		g.rect(0, 0, this.width, g.textSize() + this.padding);
		g.fill(255);
		g.text(this.name, this.padding, this.padding);
		g.pop();
	}

	/**
	 *
	 * @param {import("types/p5").Graphics} g
	 */
	drawSocketText(g) {
		g.push();
		g.textSize(13);
		g.textAlign(LEFT, CENTER);
		g.fill(255);
		let socketBaseline = 14 + this.padding * 2.5;
		for (let i in this.inputSockets) {
			let ypos = socketBaseline + 23 * i;
			let socket = this.inputSockets[i];
			g.text(socket.name, this.padding + 6, ypos);
			socket.pos = createVector(0, ypos);
		}
		g.textAlign(RIGHT, CENTER);
		for (let i in this.outputSockets) {
			let ypos = socketBaseline + 23 * i;
			let socket = this.outputSockets[i];
			g.text(socket.name, this.width - this.padding - 6, ypos);
			socket.pos = createVector(this.width, ypos);
		}
		g.pop();
	}
}
