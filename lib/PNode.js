...
/**
 * @typedef {Object} SerializedPNode
 * @property {string} uuid
 * @property {string} name
 * @property {SerializedBoundingBox} boundingBox
 * @property {string} nodeDefPath
 */

class PNode {
	/**
	 *
	 * @param {boolean} noAutoUpdateSize Disables updateSize() automatically being called in the constructor
	 */
	constructor(noAutoUpdateSize = false) {
	...
	serialize() {
		return {
			...
			nodeDefPath: this.nodeDefPath
		};
	}

	/**
	 *
	 * @param {SerializedPNode} data
	 * @returns {PNode}
	 */
	static createFromData(data) {
		let out = NodeDef.defMap[data.nodeDefPath].createNode(false, false);
		out.uuid = data.uuid;
		out.boundingBox = BoundingBox.createFromData(data.boundingBox);
		out.shouldRepaint = true;
		out.updateSize();
		out.updateSockets();
		return out;
	}
	...
	updateSockets() {
		this.allSockets = concat(this.inputSockets, this.outputSockets);
		for (let socket of this.allSockets) {
			// Done to set socket positions
			PNode.socketDraw(
				this,
				socket,
				createVector(socket.io == "i" ? -1 : 1, 0).rotate(
					this.boundingBox.rot
				)
			);
		}
	}
	...
	static socketDraw(node, socket, fwd) {
		let drawPos = node.boundingBox.localPosCorner(socket.pos);
		socket.globalPos = drawPos;
		socket.forward = fwd;
		if (node.workspace && node.workspace.zoom < 0.25) {
			return;
		}
		//ellipse(drawPos.x, drawPos.y, 10, 10, 8)
		//rect(drawPos.x - 5, drawPos.y - 5, 10, 10);
		let rad = 5
		let detail = ceil(map(workspace.zoom, 1, 5, 8, 32))
		//let detail = ceil(((workspace.zoom+1)/4*24)-4);
		//detail = 4
		let step = 2*Math.PI/detail
		beginShape()
		for (let i = 0; i < detail; i++) {
			let angle = i*step+node.boundingBox.rot
			let x = cos(angle)*rad
			let y = sin(angle)*rad
			vertex(drawPos.x+x, drawPos.y+y)
		}
		endShape(CLOSE)
	}
	...
	drawSocketPoints() {
		...
		stroke(0);
		strokeWeight(1);
		let nodeForward = createVector(1, 0);
		nodeForward.rotate(this.boundingBox.rot);
		for (let socket of this.allSockets) {
			// i don't like copying the vector here but whatever
			PNode.socketDraw(
				this,
				socket,
				socket.io == "i"
					? nodeForward.copy().mult(-1)
					: nodeForward.copy()
			);
		}
		pop();
	...
	autoSize() {
		push()
		textSize(14)
		let w = textWidth(this.name) + this.padding*2
		let mostSockets = max(this.inputSockets.length, this.outputSockets.length)
		let h = 14 + this.padding * 4.5 + (mostSockets-1)*23
		
		textSize(13)
		let widestInput = 0
		let widestOutput = 0
		for (let input of this.inputSockets) {
			let inputLength = textWidth(input.name)
			if (inputLength > widestInput) {
				widestInput = inputLength
			}
		}
		for (let output of this.outputSockets) {
			let outputLength = textWidth(output.name)
			if (outputLength > widestOutput) {
				widestOutput = outputLength
			}
		}
		w = max(w, widestInput+widestOutput+this.padding*6)
