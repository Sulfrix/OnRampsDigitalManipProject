...
class WorkspacePart {
    ...
    snapToPart(otherPart, testOnly) { // ran when releasing a part to check for something to attach to
        let attachTo;
        let attachFrom;
        let a;
        ...
        for (let i = 0; i < this.attachments.length && doLoop; i++) {
            ...
            if (!a.parent) {
                ...
                if (nearby.length > 0) {
                    if (!otherPart || nearby[0].part == otherPart) {
        ...
        if (attachTo && attachFrom && !testOnly) {
            attachFrom.snapTo(attachTo);
        }
        return {from: attachFrom, to: attachTo}
    }
