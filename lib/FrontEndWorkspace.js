...
class FrontEndWorkspace extends Workspace {
    ...
    draw() {
        ...
        this.drawCursorPart();
        if (this.cursorPart && this.mouseArea.pointIntersects(mouseX, mouseY)) {
     ...
     mouseEvent(p, button, e) {
        ...
        if (p && this.currTab) {
            for (let i of this.currTab.parts) {
                if (i.boundingBox.pointIntersects(mouseX, mouseY)) {
                    this.grabOnCursor(i.instantiatePart());
                    this.cursorPartPos = createVector(i.pos.x, i.pos.y);
                    this.cursorPartSize = createVector(this.sidebarWidth, PartListing.drawHeight-PartListing.textHeight);
                }
            }
        }
    }
...
class PartListing {
    ...
    draw(g, w) {
        ...
        if (!this.initialized) {
            ...
            if (!this.imageReady) {
                ...
                let size = rectFit(w-10, imageHeight-10, this.image.width, this.image.height);
    ...
    instantiatePart() {
        return new WorkspacePart(0, 0, this.part)
    }
}
