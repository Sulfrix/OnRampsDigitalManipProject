        ...
        if (p && this.currTab) {
            for (let i of this.currTab.parts) {
                if (i.isHovered()) {
                    ...
                    this.cursorPartSize = createVector(this.sidebarWidth-10, PartListing.drawHeight-PartListing.textHeight);
                }
            }
        }
    }
    ...
    draw(g, w) {
        ...
        if (!this.initialized) {
            ...
            if (!this.imageReady) {
                ...
                let size = rectFitEx(w-10, imageHeight-10, this.image.width, this.image.height);
                g.image(this.image, w / 2 - size.w / 2, size.offsetY, size.w, size.h);
                g.pop();
            }
            push();
