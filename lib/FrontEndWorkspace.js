...
class FrontEndWorkspace extends Workspace {
    constructor() {
        ...
        var butt = new OverlayButton("👁️", "Hide Sidebar", () => {
            if (this.targetSidebarSlide == 1) {
                butt.name = "Show Sidebar"
                this.targetSidebarSlide = 0
            } else {
                butt.name = "Hide Sidebar"
                this.targetSidebarSlide = 1
            }
        }, 50, "v");
        this.buttons.unshift(butt);
    }
    ...
    get mouseArea() { // The mouse area is smaller so we don't grab parts that are behind another UI element (the sidebar)
        if (!this.cacheMouseArea) {
            this.cacheMouseArea = new BoundingBox(createVector((width / 2) + this.sidebarWidth*this.sidebarSlide / 2, height / 2), (width) - this.sidebarWidth*this.sidebarSlide, height, 0)
        }
    ...
    draw() {
        var slide = lerp(this.sidebarSlide, this.targetSidebarSlide, 1 - exp(-10.0*(deltaTime/1000.0)))
        if (slide != this.sidebarSlide) {
            this.sidebarSlide = slide
            this.invalidateMouseArea()
        }
        super.draw()
        push();
        translate(-this.sidebarWidth*(1-this.sidebarSlide), 0)
        push();
        noStroke();
        ...
        { // Each of the animation timers need to be updated and clamped, these braces are here so it can be collapsed in a code editor
    ...
    drawTabs() { // Lots of layout code so these tabs draw left-to-right, top-to-bottom.
        ...
        for (let x of this.tabs) {
            ...
            noStroke();
            noFill();
            if (mouseX+(this.sidebarWidth*(1-this.sidebarSlide)) > posX && mouseX+(this.sidebarWidth*(1-this.sidebarSlide)) < posX + boxwidth) {
...
class PartListing { // What the part list is drawing
    ...
    isHovered(tabheight = this.tabheight) {
        if (this.boundingBox.pointIntersects(mouseX+(workspace.sidebarWidth*(1-workspace.sidebarSlide)), mouseY) && mouseY > tabheight) {
            return true;
        }
        return false;
    }
