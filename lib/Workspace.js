...
class Workspace {
    constructor() {
        ...
        this.buttons.push(new OverlayButton("❓", "Help (Not Ready)", () => {}, 50, "h"));
        this.buttons.push(new OverlayButton("🗑", "Delete All", () => {
    ...
    update() {
        ...
        if (pInput.rightDown() && this.validMousePoint(mouseX, mouseY)) {
    ...
    mouseEvent(p, button, e) {
        ...
        if (p && this.getHoveredButton()) {
            this.shouldClick = this.getHoveredButton();
        }

        if (!p && this.getHoveredButton()) {
            if (this.getHoveredButton() == this.shouldClick) {
                this.shouldClick.callback();
            }
        }
