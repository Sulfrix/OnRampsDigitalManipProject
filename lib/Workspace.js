...
class Workspace {
	...
	get mouseArea() {
		// This is the area of the workspace that counts as "onscreen." This was created to account for the sidebar in our app.
	 ...
	 */
	keyEvent(key, e) {
		// Only used for button keybindings
		for (let i of this.buttons) {
			if (i.key == key) {
				i.flash();
				i.callback();
	...
	validMousePoint(x, y) {
		// Whether or not the mouse is in the workspace. This is defined here so that you can't click on something behind the buttons.
